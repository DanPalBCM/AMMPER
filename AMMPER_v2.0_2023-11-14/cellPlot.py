...
def get_fixed_mins_maxs(mins, maxs):
    """
    Adjust the given minimum and maximum values and return a list with modified values.

    Parameters:
    - mins (float): The minimum value.
    - maxs (float): The maximum value.

    Returns:
    - list: A list containing modified minimum and maximum values.
    """
    deltas = (maxs - mins) / 12.
    mins = mins + deltas / 4.
    maxs = maxs - deltas / 4.
...
def cellPlot(data,gen,radData,ROSData,radGen,N,plots_dir):
    """
    Generate 3D scatter plots for cell data.

    Parameters:
    - data (list): A list containing cell data.
    - gen (int): The generation number.
    - radData (numpy.ndarray): Numpy array containing radiation data.
    - ROSData: Data for Reactive Oxygen Species (ROS).
    - radGen: Generation number for radiation data.
    - N (float): A parameter for setting plot limits.
