...
def GammaRadGen(dose):
    """
    Generates simulated radiation data for a given dose using a gamma radiation model.

    Args:
        dose (float): Radiation dose in Gy (Gray).

    Returns:
        np.ndarray: Array containing simulated radiation data.
    """
    import numpy as np
    import pandas as pd
